...
onMounted(async () => {
  ...
  // setView瞬间相机到达
  //单一颜色
  let material = new Cesium.ColorMaterialProperty(new Cesium.Color(1, 1, 1, 0.5))
  // 条纹
  ...
  // 间隔
  const material3 = new Cesium.StripeMaterialProperty({
    evenColor: Cesium.Color.WHITE,
    oddColor: Cesium.Color.BLACK,
    repeat: 32,
  })
  // 网格
  const material4 = new Cesium.GridMaterialProperty({
  ...
  handler.setInputAction(function (movement) {
    ...
    if (Cesium.defined(pickedObject)) {
      let attribute
      console.log(typeof pickedObject.id)
      if (typeof pickedObject.id === 'object') {
        pickedObject.id.rectangle.material = Cesium.Color.YELLOW.withAlpha(0.5)
      }
      if (typeof pickedObject.id === 'string') {
        attribute = primitive.getGeometryInstanceAttributes(pickedObject.id)
        attribute.color = Cesium.ColorGeometryInstanceAttribute.toValue(Cesium.Color.YELLOW.withAlpha(0.5))
      }
    }
